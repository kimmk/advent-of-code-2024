...
fn calc_ops(
    ...
    let cat = format!("{}{}", nums[0], nums[1]).parse::<i64>().unwrap();
    if nums.len() == 2 {
        ret |= sum == result;
        ret |= pro == result;
        ret |= cat == result;
        ...
        let nums_rest = &nums[2..];
        let mut sum_nums = vec![sum];
        sum_nums.extend_from_slice(nums_rest);
        ret |= calc_ops(&sum_nums, result);
        let mut pro_nums = vec![pro];
        pro_nums.extend_from_slice(nums_rest);
        ret |= calc_ops(&pro_nums, result);
        let mut cat_nums = vec![cat];
        cat_nums.extend_from_slice(nums_rest);
        ret |= calc_ops(&cat_nums, result);
    }
    return ret;
}
